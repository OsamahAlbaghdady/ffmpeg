    ...
    function (Request $request) {
        ...
        if ($request->files) {
            ...
            foreach ($request->file('files') as $file) {
                ...
                $savedPathConvert =  base_path() . '/public/Attachments/' ."con". $name;
		...
		// $options = "-vf scale=$resolution -c:v h264_nvenc -cq 23 -b:v 500k -c:a copy";
                ...
                echo exec("ffmpeg -hwaccel cuvid -i $ff -c:v h264_nvenc -preset slow -crf 22 -c:a copy $savedPathConvert");
                return response()->json(['url' => url('/Attachments/' . $name) , 'link' => $savedPathConvert]);

                // $command = "ffmpeg -i $savedPathConvert -c:v h264_nvenc -pix_fmt yuv420p -vf \"scale=$resolution,zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p\" -b:v $bitRate -c:a copy $savedPath";
                // echo exec($command);//

                // unlink($savedPathConvert);
